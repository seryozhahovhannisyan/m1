module.exports = function(grunt) {
	...
	grunt.registerMultiTask('lcpGeneratorTransaction', 'lcp generator transaction', function () {
		grunt.log.write('transactionGenerator Generator start...').ok();
		var fs = require('fs'),
			done = this.async();
		...
		grunt.util.async.forEach(this.files, function (filePair, filePairDone) {
			...
			grunt.util.async.forEach(messages, function (relativeFilePath, fileBuildDone) {
				...
				grunt.file.write(destination   + filename + '.js', getContent(template, content, filename, fields), 'utf8');
				fileBuildDone();
			}, filePairDone);
		}, done);
	});

	var messages_dest = './../merchant/src/main/java/com/connectto/wallet/merchant/common/data/transaction/lcp';
